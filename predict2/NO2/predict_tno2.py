...
DATADIR = "./Desktop/BDP/predict2/NO2/data"
 # data directory
...
sc.addPyFile("Desktop/BDP/predict2/NO2/pyspark_csv.py")
...
if __name__ == "__main__":
    ...
    train = loadDF("trainNO2.csv")
    test = loadDF("test.csv")

    testNumber = test.select('Number').rdd.map(lambda x: x.Number)

    train = train.select('NO2 TAQI', 'NO2 AQI', 'O3 AQI', 'SO2 AQI', 'CO AQI')
    ...
    submit = sqlCtx.createDataFrame(testNumber.zip(pred), ["Number", "SO2 TAQI"])
    """
